...
file = open('2048_score.txt', 'r')
init_high_score = file.read()
a = int(init_high_score)
file.close()
high_score = a
...
def board():
    pygame.draw.rect(screen, colors['background'], [0, 0, 400, 400], 0, 20)
...
def pieces(board):
    for i in range(4):
        for j in range(4):
            ...
            # color of number in pieces
            if value > 8:
                value_color = colors['light text']
            else:
                value_color = colors['dark text']

            # color of pieces background
            if value <= 2048:
                color = colors[value]
            else:
                color = colors['other']
            ...
            if value > 0:
                ...
                font = pygame.font.Font(None, 48 - (5*value_len))
...
def draw_game_over():
    ...
    screen.blit(game_over_text, (150, 70))
    screen.blit(restart_text, (108, 105))
...
while run:
    ...
    if spawn_new or init_count < 1:
        board_value, game_over = new_pieces(board_value)
        spawn_new = False
        init_count += 1
